...
hbg.addEventListener('click', () => { hbg.classList.toggle('on'); nm.classList.toggle('open'); });
function closeNav() { hbg.classList.remove('on'); nm.classList.remove('open'); }
...
async function doSend(form, stEl, subject) {
  ...
  try {
    ...
    if (result.success) {
      showStatus(stEl, 'ok', `✓ Message sent! We'll reply to ${data.email} within 24 hours.`);
      form.reset();
